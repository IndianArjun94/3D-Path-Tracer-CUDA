...
__device__ float3 getLocalColor(float3 rayPos, float3 rayDir, Material material, float3 normal, Triangle* triangles, int numTriangles, Sphere* spheres, int numSpheres, PointLight* pointLights, int numPointLights, unsigned int& seed) {
    ...
    for (int i = 0; i < numPointLights; i++) {
        ...
        seed = hash(seed);
        float rx = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
        seed = hash(seed);
        float ry = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
        seed = hash(seed);
        float rz = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
...
__global__ void sample(uchar4* pbo, double4* accumulationBuffer, int width, int height, Triangle* triangles, int numTriangles, Sphere* spheres, int numSpheres, PointLight* pointLights, int numPointLights, int frame_number) {
    ...
    seed = hash(seed);
    float jitterX = (fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f) / (float)width;

    seed = hash(seed);
    float jitterY = (fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f) / (float)height;
    ...
    for (int bounces = 0; bounces < 10; bounces++) {
        ...
        seed = hash(seed);
        float specularRandom = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f));
        ...
        if (specularRandom < P_spec) { // specular
            ...
            if (material.roughness != 0) {
                seed = hash(seed);
                float randomFloatX = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
                seed = hash(seed);
                float randomFloatY = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
                seed = hash(seed);
                float randomFloatZ = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;

                float3 random = make_float3(randomFloatX, randomFloatY, randomFloatZ);
                while (length(random) > 1) {
                    randomFloatX = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
                    seed = hash(seed);
                    randomFloatY = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;
                    seed = hash(seed);
                    randomFloatZ = fmaxf(0.001, fminf(0.999, (float)seed / 4294967295.0f)) * 2.0f - 1.0f;

                    random = make_float3(randomFloatX, randomFloatY, randomFloatZ);
                }
